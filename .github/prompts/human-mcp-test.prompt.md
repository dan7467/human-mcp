...
First - start the MCP server by running `npm start` in the terminal. Then, I will use the `curl` command to invoke different tools such as `enter_text`, `single_click_mouse`, and `search_point_or_area_on_screen`. For example, I can test the mouse click tool by sending a POST request to the `/invoke` endpoint with the appropriate JSON payload.
